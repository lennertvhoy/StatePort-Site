# NEXT_ACTIONS - active execution queue

**Updated At:** 2026-09-02
**Execution Mode:** operating
**Max Items:** 1

## P0 [BL-ALPHA11-PODMAN-CLEAN-INSTALL] Owner public-path test of published Alpha.12

**Status:** Alpha.12 is published, signed, install-enabled, and presented on
the site as the current public-test candidate. Alpha.11 is superseded and
install-disabled (retained as history); Alpha.10 remains rejected and
install-disabled.

**Decision:** Alpha.12 is the current public-test candidate; the site release
truth and validators now bind Alpha.12. The owner public-path install test and
human verdict remain.

**Exit:** Owner clean-install receipt on a real Windows 11 + WSL2 + Ubuntu
24.04 host, then the human verdict. Human acceptance remains separate.

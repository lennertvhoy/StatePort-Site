...
refused in the immutable installer package preflight because the bootstrap
had not staged signature bundles into their digest slots; the mutable route
now carries that digest-slot transport repair over the unchanged versioned
bootstrap. Alpha.11 is superseded and install-disabled (retained as history);
Alpha.10 remains rejected and install-disabled.
...
24.04 host after the mutable-route repair, then the human verdict. Human

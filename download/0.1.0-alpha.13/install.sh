#!/bin/sh
# StatePort v0.1.0-alpha.13 Windows 11 + WSL2 + Ubuntu 24.04 bootstrap.
set -eu
STATEPORT_VERSION="0.1.0-alpha.13"
RELEASE_ROOT="https://lennertvhoy.github.io/StatePort-Site/download/0.1.0-alpha.13"
...
check "ed8eb6cbb6cd828b7abb361f9b81976fd3892ad303a078a849817733669d8727" "$tmp/installer"
get "$RELEASE_ROOT/stateport-execution-host-provision" "$tmp/provisioner" "execution-host provisioner"
check "312ef592fb05bb45458b8f58e5551f0e5594bde2111d90555267494785da6d32" "$tmp/provisioner"
...
check "cbb5cc93d463650c0675dacf0f9ba42063adc17c11bcedf360d9a29be12d5405" "$tmp/release-index.json"
get "$RELEASE_ROOT/release-index.sigstore.json" "$tmp/release-index.sigstore.json" "release index signature"
check "492fa65fd88cc4d7e719fb52d87d5f582eb65219586170e31f505a892ef85e74" "$tmp/release-index.sigstore.json"
...
sudo -n sh -c 'printf "%s  %s\n" "$1" "$2" | sha256sum -c --status' sh "ed8eb6cbb6cd828b7abb361f9b81976fd3892ad303a078a849817733669d8727" "$root_stage/installer" || fail "Sealed installer copy changed."
...
sudo -n sh -c 'printf "%s  %s\n" "$1" "$2" | sha256sum -c --status' sh "cbb5cc93d463650c0675dacf0f9ba42063adc17c11bcedf360d9a29be12d5405" "$root_stage/release-index.json" || fail "Sealed release index changed."
sudo -n sh -c 'printf "%s  %s\n" "$1" "$2" | sha256sum -c --status' sh "492fa65fd88cc4d7e719fb52d87d5f582eb65219586170e31f505a892ef85e74" "$root_stage/release-index.sigstore.json" || fail "Sealed release signature changed."

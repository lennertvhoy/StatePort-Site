...
any model, vendor, or interface — a place where ongoing work lives, with
named state, history, goals, and continuity over time. Second, everything
the user sees — the conversation, the previews, the dashboards — is a
projection of that state, never the truth itself. Third, the state is
protected by a governance layer: consequential mutations are proposed,
approved where the consequences justify it, validated, applied, and
recorded, with a receipt that can be inspected later. The order of these
claims is deliberate. The application and its ongoing work come first;
governance is the enabling layer that keeps that work safe and inspectable,
...
the terminal, the automation — is an instrument acting on that
state under explicit rules, replaceable only where the contracts permit it.
...
plus a governed lifecycle, independent of any single agent or model provider.

## Revision note for version 1.2

Version 1.2 keeps the argument of 1.1 intact and corrects eleven points of
emphasis and accuracy learned from building the reference implementation:
...
4. **Both product planes are valid.** Stateware has an application plane
   (durable, user-owned stateful applications operating over time) and an
   execution plane (managed coding-agent harnesses with supervision,
...
## 3.6 The application plane and the execution plane

Stateware operates on two planes, and keeping them distinct prevents a
common misreading. The **application plane** is where durable, user-owned
applications operate over time: named state, history, goals, context, and
continuity — a study coach, a checklist, a domain assistant. The
**execution plane** is where managed coding-agent harnesses run:
opinionated providers, supervision, permissions, isolation, cancellation,
and evidence. The planes cooperate — the execution plane does work that
...
provider, with its own sessions, processes, and operational context. The
definition says what the application is; the instance says what has happened
to this one; the realization is merely where and how it happens to be
running right now. Realizations are disposable in exactly the way instances
are not: lose the container and the instance can be re-realized elsewhere
...
pretend to be the whole system.
...
does not need to be rebuilt around it: the canonical state and its contracts
...
old one. Loyalty to an assistant no longer requires loyalty to a vendor;
...
This division draws an explicit **ownership seam**. The platform owns
...
genuinely required, and stripped from everywhere else. This is what it
means to call governance an **enabling layer**: it is what makes
unsupervised-safe work possible and supervised work honest, not a tax on
either. A system that asks precisely when it should ask is not slower than
one that never asks. It is merely honest about who is in charge.
...
**Engines as profiled providers.** Models improve, specialize, and
commoditize. In a Stateware ecosystem this is upside, because changing
...
brilliant they become — hired, profiled, and never in charge of the
